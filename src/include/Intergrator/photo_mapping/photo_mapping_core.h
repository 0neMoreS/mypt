...
struct Photon {
    ...
    Photon() {}
    Photon(const Vec3f& throughput, const Vec3f& position, const Vec3f& wi) : throughput(throughput), position(position), wi(wi) {}
};
...
private:
    ...
    void searchKNearestNode(int nodeIdx, const PointU& queryPoint, int k, KNNQueue& queue) const {
...
public:
    ...
    template <typename PointU>
        requires Point<PointU>
    std::vector<int> searchKNearest(const PointU& queryPoint, int k, float& maxDist2) const {
        KNNQueue queue;
...
public:
    ...
    std::vector<int> queryKNearestPhotons(const Vec3f& p, int k, float& max_dist2) const {
